...
bIbWNoRm <-
            ...
            BPPARAM = SerialParam())
  {
    ...
    if ((sum(do.call(c, SeqDepthGrouped) == 10000) / (nrow(Data) *
                                                      ncol(Data))) >= 0.8) {
      warning(
        "More than 80% of your data is zeros.  \n        Check your data (remove low quality cells prior). \n
            You may need to adjust the filtering criteria \n        parameters FilterExpression and FilterCellNum. \n
    ...
    message("Gene filtering is applied within each cluster.")
    ...
    MedExprGrouped <- lapply(seq_along(Levels), function(x) {
      apply(GroupData[[x]], 1, function(c)
        median(c[c != 0]))
    })
